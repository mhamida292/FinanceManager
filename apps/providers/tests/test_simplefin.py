...
@responses.activate
def test_fetch_investment_accounts_handles_robinhood_style_payload():
    """Robinhood via SimpleFIN: no `price` field, `cost_basis` is always '0.00',
    but `purchase_price` carries the real per-share cost."""
    access_url = "https://U:T@bridge.simplefin.org/simplefin"
    responses.add(
        responses.GET,
        f"{access_url}/accounts?start-date=0",
        json={
            "accounts": [
                {
                    "id": "INV-RH", "name": "Robinhood individual",
                    "currency": "USD", "org": {"name": "Robinhood"},
                    "holdings": [
                        {
                            "id": "H-MU",
                            "symbol": "MU",
                            ...
                            "shares": "19.108756",
                            "market_value": "9477.942976",
                            "purchase_price": "131.466",
                            "cost_basis": "0.00",
                        },
                    ],
                },
    ...
    inv_payloads = list(SimpleFINProvider().fetch_investment_accounts(access_url))
    assert len(inv_payloads) == 1
    ...
    assert mu.cost_basis == expected_cb
